#pragma once
    ...
    (defined(_M_X64) && !defined(_M_ARM64EC)) || defined(__i386__) ||          \
    defined(_M_IX86) || defined(__e2k__)
...
template <typename T> T popcnt(T n) {
...
inline uint32_t lzcnt(uint64_t n) {
  ...
  return 63 - bsr;
#elif defined(DXVK_ARCH_X86_64) &&                                             \
    ((defined(_MSC_VER) && !defined(__clang__)) && defined(__LZCNT__))
  return _lzcnt_u64(n);
#elif defined(DXVK_ARCH_X86_64) && (defined(__GNUC__) || defined(__clang__))
...
#elif defined(DXVK_ARCH_ARM64) && (defined(__GNUC__) || defined(__clang__))
...
#pragma GCC unroll 0
  ...
  for (size_t i = 0; i < size; i += 64u) {
    auto *ptr =
        reinterpret_cast<uint64x2_t *>(reinterpret_cast<char *>(mem) + i);
    vst1q_u64(reinterpret_cast<uint64_t *>(ptr + 0u), zero);
    vst1q_u64(reinterpret_cast<uint64_t *>(ptr + 1u), zero);
    vst1q_u64(reinterpret_cast<uint64_t *>(ptr + 2u), zero);
    vst1q_u64(reinterpret_cast<uint64_t *>(ptr + 3u), zero);
  }
#else
...
#pragma GCC unroll 0
  ...
  return true;
#elif defined(DXVK_ARCH_ARM64) && (defined(__GNUC__) || defined(__clang__))
  auto ai = reinterpret_cast<const uint8_t *>(a);
  auto bi = reinterpret_cast<const uint8_t *>(b);
...
#pragma GCC unroll 0
  ...
  for (; i < 2 * (sizeof(T) / 32); i += 2) {
    uint8x16_t a0 = vld1q_u8(ai + (i + 0) * 16);
    uint8x16_t b0 = vld1q_u8(bi + (i + 0) * 16);
    uint8x16_t a1 = vld1q_u8(ai + (i + 1) * 16);
    uint8x16_t b1 = vld1q_u8(bi + (i + 1) * 16);

    uint8x16_t res = veorq_u8(a0, b0) | veorq_u8(a1, b1);
    if (vmaxvq_u8(res) != 0)
      return false;
  }

  for (; i < sizeof(T) / 16; i++) {
    uint8x16_t res = veorq_u8(vld1q_u8(ai + i * 16), vld1q_u8(bi + i * 16));
    if (vmaxvq_u8(res) != 0)
      return false;
  }
...
public:
  ...
  public:
    ...
    explicit iterator(T flags) : m_mask(flags) {}
    ...
    T operator*() const { return bsf(m_mask); }
 ...
 */
template <typename T, int32_t I, int32_t F> T encodeFixed(float n) {
  if (n != n)
    return 0u;
 ...
 */
template <typename T, int32_t I, int32_t F> float decodeFixed(T n) {
 ...
 */
inline uint64_t fnv1a_init() { return 0xcbf29ce484222325ull; }
...
inline uint64_t fnv1a_hash(const unsigned char *data, size_t size) {
  ...
  while (idx + sizeof(hash) <= size) {
    uint64_t v =
        (uint64_t(data[idx + 0u]) << 0u) | (uint64_t(data[idx + 1u]) << 8u) |
        (uint64_t(data[idx + 2u]) << 16u) | (uint64_t(data[idx + 3u]) << 24u) |
        (uint64_t(data[idx + 4u]) << 32u) | (uint64_t(data[idx + 5u]) << 40u) |
        (uint64_t(data[idx + 6u]) << 48u) | (uint64_t(data[idx + 7u]) << 56u);
