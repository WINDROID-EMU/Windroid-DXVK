...
DxvkPipelineWorkers::~DxvkPipelineWorkers() { this->stopWorkers(); }

void DxvkPipelineWorkers::compilePipelineLibrary(
    DxvkShaderPipelineLibrary *library, DxvkPipelinePriority priority) {
...
void DxvkPipelineWorkers::compileGraphicsPipeline(
    DxvkGraphicsPipeline *pipeline, const DxvkGraphicsPipelineStateInfo &state,
...
void DxvkPipelineWorkers::startWorkers() {
  if (!std::exchange(m_workersRunning, true)) {
...
#if defined(__ANDROID__) || defined(DXVK_ARCH_ARM64)
    if (workerCount > 1 && m_device->config().numCompilerThreads <= 0)
      workerCount = std::max(1u, (workerCount * 3) / 4);
#endif
    ...
    for (size_t i = 0; i < workerCount; i++) {
      ...
      auto &worker =
          m_workers.emplace_back([this, priority] { runWorker(priority); });
...
void DxvkPipelineWorkers::runWorker(DxvkPipelinePriority maxPriority) {
  ...
  env::setThreadName(
      str::format("dxvk-shader-", suffixes.at(maxPriorityIndex)));
...
DxvkPipelineManager::DxvkPipelineManager(DxvkDevice *device)
  ...
  Logger::info(str::format("DXVK: Graphics pipeline libraries ",
                           (m_device->canUseGraphicsPipelineLibrary()
                                ? "supported"
                                : "not supported")));
...
DxvkComputePipeline *DxvkPipelineManager::createComputePipeline(
  ...
  auto iter =
      m_computePipelines.emplace(std::piecewise_construct, std::tuple(shaders),
...
DxvkGraphicsPipeline *DxvkPipelineManager::createGraphicsPipeline(
  ...
  auto iter = m_graphicsPipelines.emplace(
      std::piecewise_construct, std::tuple(shaders),
...
DxvkPipelineManager::createVertexInputLibrary(
  ...
  auto iter = m_vertexInputLibraries.emplace(
      std::piecewise_construct, std::tuple(state), std::tuple(m_device, state));
  return &iter.first->second;
}

DxvkGraphicsPipelineFragmentOutputLibrary *
DxvkPipelineManager::createFragmentOutputLibrary(
...
void DxvkPipelineManager::registerShader(const Rc<DxvkShader> &shader) {
  DxvkShaderPipelineLibraryKey key;
...
void DxvkPipelineManager::requestCompileShader(const Rc<DxvkShader> &shader) {
...
void DxvkPipelineManager::stopWorkerThreads() { m_workers.stopWorkers(); }
...
const DxvkDescriptorSetLayout *DxvkPipelineManager::createDescriptorSetLayout(
  ...
  auto iter = m_descriptorSetLayouts.emplace(
      std::piecewise_construct, std::tuple(key), std::tuple(m_device, key));
  return &iter.first->second;
}

const DxvkPipelineLayout *
DxvkPipelineManager::createPipelineLayout(const DxvkPipelineLayoutKey &key) {
  std::lock_guard<dxvk::mutex> lock(m_layoutMutex);
...
DxvkShaderPipelineLibrary *DxvkPipelineManager::createPipelineLibraryLocked(
    const DxvkShaderPipelineLibraryKey &key) {
  auto iter =
      m_shaderLibraries.emplace(std::piecewise_construct, std::tuple(key),
...
DxvkShaderPipelineLibrary *DxvkPipelineManager::createNullFsPipelineLibrary() {
  ...
  auto iter = m_shaderLibraries.emplace(std::piecewise_construct, std::tuple(),
                                        std::tuple(m_device, this, key));
  return &iter.first->second;
}
